Feature: Set ‘Route requisitions to buyer-if no contract is specified-when non-backed line exceeded’ in company information page and submit a requisition with the item (that had no contract) such that the total exceeds the specified amount.
...
Background:I am opening the home page of coupa
 ...
 Scenario:Creating requisition to buyer with supplier that have no contract
         ...
         And when non-backed lines exceeded setting amount
